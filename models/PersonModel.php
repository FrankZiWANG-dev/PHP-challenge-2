<?php
	...
	class PersonModel
	{
		...
		public function getAddPerson()
		{
			...
			if (isset($_POST["submit"])) {
				...
				if (filter_var($firstname, FILTER_SANITIZE_STRING)) {
					...
					$error['0'] = "error on firstname";
					$errorMessage['0'] = "mauvaise firstname";
				}
				
				if (filter_var($lastname, FILTER_SANITIZE_STRING)) {
					$lastname_valid = $lastname;
				} else {
					$error['1'] = "error on lastname";
					$errorMessage['1'] = "mauvaise lastname";
				}
				...
				if (filter_var($company_san, FILTER_VALIDATE_INT)) {
					...
					$error['4'] = "error on company";
					$errorMessage['4'] = "mauvaise société";
				}
				if (isset($error)) {
					$addRequestSQL =
		...
		public function getUpdateDetailPerson()
		{
			require "assets/config/php/config.php";
			$id = intval($_GET['id']);
			$requestSQL =
